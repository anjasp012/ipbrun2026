    ...
    <div class="fixed inset-0 bg-[#001A33] flex items-center justify-center p-6 select-none overflow-hidden">
        ...
        <div class="relative w-full max-w-lg" x-data="startTool()">
            ...
            <template x-if="state === 'password'">
                <div class="max-w-md mx-auto text-center space-y-10" x-transition:enter="transition ease-out duration-500"
                    x-transition:enter-start="opacity-0 translate-y-8"
                    x-transition:enter-end="opacity-100 translate-y-0">
                    <img src="{{ asset('assets/images/logo_ipbrun2026.png') }}" class="h-20 mx-auto drop-shadow-xl"
                        alt="">

                    <div class="space-y-4">
                        <label class="text-[10px] font-black text-[#FF7A21] uppercase tracking-[5px] block">SECURITY
                            CLEARANCE</label>
                        <input type="password" x-model="password" @focus="playStandby()" @keyup.enter="checkPass()"
                            placeholder="••••••••••••"
                        ...
                        <template x-if="error">
                            <p class="text-xs font-bold text-red-500 uppercase tracking-widest italic" x-text="error">
                            </p>
                        </template>
                    </div>
            ...
            <template x-if="state === 'ready'">
                <div class="text-center space-y-12" x-transition:enter="transition ease-out duration-500"
                    ...
                    <div class="space-y-4">
                        ...
                        <p class="text-[11px] font-bold text-white/40 uppercase tracking-[4px]">PRESS START TO TRIGGER
                            COUNTDOWN</p>
                    </div>
