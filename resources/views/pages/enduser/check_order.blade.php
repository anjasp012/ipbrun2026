    ...
    <div class="py-12 flex items-center justify-center p-4 min-h-screen">
        <div class="max-w-4xl mx-auto w-full">
            ...
            <div class="text-center mb-8">
                <img src="{{ asset('assets/images/logo_ipbrun2026.png') }}" alt="IPB Run 2026" class="h-20 mx-auto mb-6 drop-shadow-xl">
            ...
            <!-- Results Section -->
            @if($order)
                <div class="bg-white/95 rounded-3xl shadow-2xl border border-white/20 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <!-- Order Header -->
                    <div class="bg-[#003366] p-6 text-white flex flex-col md:flex-row justify-between items-center gap-4">
                        ...
                        <div class="text-center md:text-right">
                            ...
                            @if($order->status === 'paid')
                                <span class="inline-flex px-4 py-1.5 rounded-full bg-emerald-500 text-white text-xs font-black uppercase tracking-widest shadow-lg shadow-emerald-500/30">BERHASIL (PAID)</span>
                            @elseif($order->status === 'pending')
                                <span class="inline-flex px-4 py-1.5 rounded-full bg-amber-500 text-white text-xs font-black uppercase tracking-widest shadow-lg shadow-amber-500/30">MENUNGGU (PENDING)</span>
                            @else
                                <span class="inline-flex px-4 py-1.5 rounded-full bg-red-500 text-white text-xs font-black uppercase tracking-widest shadow-lg shadow-red-500/30">GAGAL / EXPIRED</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="p-6 md:p-8">
                        <div class="grid md:grid-cols-2 gap-8">
                            <!-- Participant Info -->
                            <div>
                                <h3 class="text-sm font-black text-[#003366] uppercase tracking-widest mb-4 flex items-center gap-2">
                                    <span class="w-1 h-4 bg-[#00ACB1] rounded-full"></span>
                                    Data Peserta
                                </h3>
                                <div class="space-y-4">
                                    <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100 transition-hover hover:border-[#00ACB1]/30">
                                    ...
                                    <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Nomor WhatsApp</p>
                                    ...
                                    <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                        ...
                                        <p class="font-bold text-[#003366]">{{ Str::mask($order->participant->nik, '*', 6, 8) }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="space-y-4">
                                    @foreach($order->raceEntries as $entry)
                                        <div class="relative bg-[#003366]/5 p-5 rounded-3xl border-2 border-dashed border-[#003366]/20">
                                            <div class="flex justify-between items-start mb-2">
                                                <div>
                                                    <p class="text-[10px] font-bold text-[#00ACB1] uppercase tracking-[2px] mb-1">Kategori</p>
                                                    <p class="text-xl font-black text-[#003366] leading-none">{{ $entry->ticket->category->name }}</p>
                                                </div>
                                                <div class="text-right">
                                                    <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">BIB Number</p>
                                                    <p class="font-black text-[#003366]">{{ $entry->bib_number ?? 'TBA' }}</p>
                                                </div>
                                            </div>
                                            <div class="mt-4 pt-4 border-t border-[#003366]/10 flex justify-between items-end">
                                               <div>
                                                   <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Jersey Size</p>
                                                   <p class="font-bold text-[#003366]">{{ $order->participant->jersey_size }}</p>
                                               </div>
                                               <div class="text-right">
                                                   <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Type</p>
                                                   <p class="font-bold text-[#003366] uppercase">{{ $entry->ticket->type }}</p>
                                               </div>
                                            </div>
                                            <!-- Ticket Cut Decoration -->
                                            <div class="absolute -left-2 top-1/2 -translate-y-1/2 w-4 h-8 bg-white/95 rounded-r-full border-y border-r border-slate-200"></div>
                                            <div class="absolute -right-2 top-1/2 -translate-y-1/2 w-4 h-8 bg-white/95 rounded-l-full border-y border-l border-slate-200"></div>
                                        </div>
                                    @endforeach
                                    ...
                                    <div class="mt-6 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                        <div class="flex justify-between items-center mb-2">
                                            ...
                                            <span class="text-lg font-black text-[#003366]">Rp {{ number_format($order->total_price, 0, ',', '.') }}</span>
                                        </div>
            ...
            @elseif($error)
                <div class="bg-white/95 rounded-3xl p-12 text-center shadow-2xl border border-white/20">
                    ...
                    <h3 class="text-xl font-black text-[#003366] uppercase tracking-tight mb-2">Data Tidak Ditemukan</h3>
                    <p class="text-slate-500 font-medium mb-6">{{ $error }}</p>
                    <a href="{{ route('check.order') }}" class="inline-block text-[#00ACB1] font-bold uppercase tracking-widest text-xs hover:underline">
                        Coba Lagi
                    </a>
                </div>
            @endif
            ...
            <div class="text-center mt-8">
                <a href="/" class="text-white/60 hover:text-white font-bold uppercase tracking-[4px] text-[10px] transition-all flex items-center justify-center gap-2">
