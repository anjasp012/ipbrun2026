                    ...
                    <div class="p-6 bg-slate-50/50 border-t border-slate-50">
                        <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
                            <table class="w-full text-left">
                                <thead>
                                    <tr class="bg-slate-50 uppercase tracking-widest text-[10px] font-black text-slate-400 border-b border-slate-100">
                                        <th class="px-6 py-5">Nama Tiket</th>
                                        <th class="px-6 py-5 text-center">Harga</th>
                                        <th class="px-6 py-5 text-center">Diskon (Nominal)</th>
                                ...
                                <tbody class="divide-y divide-slate-50">
                                    ...
                                    <tr class="hover:bg-slate-50/30 transition-colors group" x-data="{ 
                                        ...
                                        <td class="px-6 py-5">
                                            <div class="font-bold text-slate-800 uppercase tracking-tight text-sm">{{ $ticket->name }}</div>
                                            <div class="text-[10px] text-slate-400 font-bold uppercase tracking-[2px] mt-1 opacity-60 italic">Kategori: {{ $ticket->category->name ?? '-' }}</div>
                                        </td>
                                        <td class="px-6 py-5 text-center">
                                            <div class="inline-flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-xl border border-slate-100 focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50 transition-all">
                                                ...
                                                <input type="number" x-model="price" @input="isDirty = true" @keyup.enter="save()"
                                                    class="w-28 bg-transparent border-none p-0 text-sm font-black text-slate-800 focus:ring-0">
                                            </div>
                                        </td>
                                        <td class="px-6 py-5 text-center">
                                            <div class="inline-flex items-center gap-2 px-3 py-2 bg-rose-50/50 rounded-xl border border-rose-100 focus-within:border-rose-300 focus-within:ring-2 focus-within:ring-rose-50 transition-all">
                                                <span @class(['text-[10px] font-black text-rose-300'])>Rp</span>
                                                <input type="number" x-model="discount" @input="isDirty = true" @keyup.enter="save()"
                                                    class="w-24 bg-transparent border-none p-0 text-sm font-black text-rose-600 focus:ring-0">
                                            </div>
                                        </td>
                                        <td class="px-6 py-5 text-center">
                                            <div class="inline-flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-xl border border-slate-100 focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50 transition-all">
                                        ...
                                        <td class="px-6 py-5 text-right flex items-center justify-end gap-3">
                                            <button @click="save()" x-show="isDirty" x-cloak
                                                class="px-5 py-2 bg-blue-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all flex items-center gap-2 shadow-sm shadow-blue-100">
                                            ...
                                            <div x-show="!isDirty" class="text-[10px] font-bold text-slate-300 uppercase italic">
                                                Terupdate
                                            </div>
                        ...
                        {{-- Quick Add Ticket --}}
                        <div class="mt-4 p-6 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                            <h5 class="text-[10px] font-black text-[#003366] uppercase tracking-widest mb-4 flex items-center gap-2">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"></path></svg>
                                Quick Add New Ticket
                            </h5>
                            <form action="{{ route('tickets.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-4 items-end">
                                @csrf
                                <input type="hidden" name="period_id" value="{{ $period->id }}">
                                
                                <div class="lg:col-span-2">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Nama Tiket</label>
                                    <input type="text" name="name" required placeholder="Contoh: 5K Regular"
                                        class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 focus:border-[#E8630A] focus:ring-4 focus:ring-orange-50 outline-none transition-all shadow-sm">
                                </div>

                                <div class="lg:col-span-1">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Kategori</label>
                                    <select name="category_id" required
                                        class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 focus:border-[#E8630A] focus:ring-4 focus:ring-orange-50 outline-none transition-all shadow-sm cursor-pointer appearance-none">
                                ...
                                <div class="lg:col-span-1">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Harga (Rp)</label>
                                    <div class="relative">
                                        ...
                                        <input type="number" name="price" required placeholder="0"
                                            class="w-full bg-white border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-sm font-bold text-slate-700 focus:border-[#E8630A] focus:ring-4 focus:ring-orange-50 outline-none transition-all shadow-sm">
                                    </div>
                                </div>

                                <div class="lg:col-span-1">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Stok</label>
                                    <input type="number" name="qty" required placeholder="0"
                                        class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 focus:border-[#E8630A] focus:ring-4 focus:ring-orange-50 outline-none transition-all shadow-sm text-center">
                                </div>

                                <div class="lg:col-span-1">
                                    <button type="submit" class="w-full py-3.5 bg-[#E8630A] text-white rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-[#cc5500] transition-all shadow-lg shadow-orange-100 flex items-center justify-center gap-2">
                                        <span>Simpan</span>
                                    </button>
                                </div>
                            </form>
