                        ...
                        <div class="flex items-center gap-6">
                            ...
                            @if(!$period->is_active)
                                <form action="{{ route('periods.toggle', $period) }}" method="POST" @click.stop="">
                                    @csrf
                                    <button type="submit" class="bg-[#003366] text-white rounded-md px-6 h-12 text-[11px] font-black uppercase tracking-widest hover:bg-[#002244] transition-all">
                                        Aktifkan Periode
                                    </button>
                                </form>
                            @endif
                                            ...
                                            <tr class="hover:bg-slate-50/30 transition-colors group" 
                                                ...
                                                <td class="px-8 py-5">
                                                    <div class="flex flex-col gap-2 w-full max-w-[250px]">
                                                        <div class="flex items-center gap-3">
                                                            <select x-model="type" @change="isDirty = true" class="bg-blue-50/50 border border-blue-100 rounded-md px-3 py-1.5 text-[11px] font-black uppercase text-blue-700 focus:ring-0">
                                                                <option value="umum">UMUM</option>
                                                                <option value="ipb">IPB</option>
                                                            </select>
                                                            <input type="text" x-model="name" @input="isDirty = true" @keyup.enter="save()" placeholder="Nama opsional..." class="bg-transparent border-none p-0 text-base font-bold text-slate-800 uppercase tracking-tight focus:ring-0 flex-grow">
                                                        </div>
                                                        <div class="text-[11px] text-slate-400 font-bold uppercase tracking-[2px] opacity-60 italic pl-1">Kategori: {{ $ticket->category->name ?? '-' }}</div>
                                                    </div>
                                                </td>
                                                <td class="px-8 py-5 text-center">
                                                    <div class="inline-flex items-center gap-3 px-4 py-2 bg-slate-50 rounded-lg border border-slate-100 focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50 transition-all">
                                                        ...
                                                        <input type="number" x-model="price" @input="isDirty = true" @keyup.enter="save()" class="w-32 bg-transparent border-none p-0 text-sm font-black text-slate-800 focus:ring-0">
                                                    </div>
                                                </td>
                                                <td class="px-8 py-5 text-center">
                                                    <div class="inline-flex items-center gap-3 px-4 py-2 bg-rose-50/50 rounded-lg border border-rose-100 focus-within:border-rose-300 focus-within:ring-2 focus-within:ring-rose-50 transition-all">
                                                        ...
                                                        <input type="number" x-model="discount" @input="isDirty = true" @keyup.enter="save()" class="w-28 bg-transparent border-none p-0 text-sm font-black text-rose-600 focus:ring-0">
                                                    </div>
                                                </td>
                                                <td class="px-8 py-5 text-center">
                                                    <div class="inline-flex items-center gap-3 px-4 py-2 bg-slate-50 rounded-lg border border-slate-100 focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50 transition-all">
                                                        <input type="number" x-model="qty" @input="isDirty = true" @keyup.enter="save()" class="w-20 bg-transparent border-none p-0 text-sm font-black text-slate-800 focus:ring-0 text-center">
                                                    </div>
                                                </td>
                                                <td class="px-8 py-5 text-center">
                                                    <span @class([
                                                ...
                                                <td class="px-8 py-5 text-right flex items-center justify-end gap-4">
                                                    <button @click="save()" x-show="isDirty" x-cloak class="px-6 py-2 bg-blue-600 text-white rounded-md text-[11px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all flex items-center gap-2 shadow-sm shadow-blue-100">
                                                    ...
                                                    <div x-show="!isDirty" class="text-[11px] font-bold text-slate-300 uppercase italic"> Terupdate </div>
                            ...
                            <div class="mt-6 p-6 bg-white rounded-xl border border-slate-100 shadow-sm">
                                <form action="{{ route('tickets.store') }}" method="POST" class="flex items-end gap-4 flex-wrap lg:flex-nowrap">
                                    ...
                                    <div class="w-40">
                                        ...
                                        <select name="type" required class="w-full bg-slate-50 border border-slate-100 rounded-lg px-5 py-3 text-sm font-bold text-slate-700 focus:bg-white focus:border-blue-300 outline-none transition-all">
                                            <option value="umum">UMUM</option>
                                            <option value="ipb">IPB</option>
                                        </select>
                                    </div>
                                    <div class="flex-grow min-w-[200px]">
                                        ...
                                        <input type="text" name="name" placeholder="Reguler / Early Bird..." class="w-full bg-slate-50 border border-slate-100 rounded-lg px-5 py-3 text-sm font-bold text-slate-700 focus:bg-white focus:border-blue-300 outline-none transition-all">
                                    </div>
                                    <div class="w-48">
                                        <label class="text-[11px] font-black text-slate-400 uppercase tracking-widest block mb-1">Kategori</label>
                                        <select name="category_id" required class="w-full bg-slate-50 border border-slate-100 rounded-lg px-5 py-3 text-sm font-bold text-slate-700 focus:bg-white focus:border-blue-300 outline-none transition-all">
                                    ...
                                    <div class="w-40">
                                        ...
                                        <input type="number" name="price" required placeholder="0" class="w-full bg-slate-50 border border-slate-100 rounded-lg px-5 py-3 text-sm font-bold text-slate-700 focus:bg-white focus:border-blue-300 outline-none transition-all">
                                    </div>
                                    <div class="w-32">
                                        <label class="text-[11px] font-black text-slate-400 uppercase tracking-widest block mb-1">Stok</label>
                                        <input type="number" name="qty" required placeholder="0" class="w-full bg-slate-50 border border-slate-100 rounded-lg px-5 py-3 text-sm font-bold text-slate-700 focus:bg-white focus:border-blue-300 outline-none transition-all text-center">
                                    </div>
