    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between bg-white p-10 rounded-2xl border border-slate-100 shadow-sm">
            <div>
                ...
                <p class="text-sm text-slate-400 font-bold uppercase tracking-wider mt-2">Kelola jendela registrasi dan
                    inventaris tiket</p>
            </div>
            <div class="flex items-center gap-4">
                <span
                    class="text-[12px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-6 py-3 rounded-lg border border-slate-100">
        ...
        @if (session('success'))
            <div
                class="bg-emerald-50 text-emerald-700 px-8 py-5 rounded-xl border border-emerald-100 text-base font-bold flex items-center gap-4 animate-slide-in">
        ...
        <div class="space-y-4" x-data="{
            activeAccordion: '{{ optional($periods->firstWhere('is_active', true))->id ?? '' }}',
            loading: false,
            ...
            @foreach ($periods as $period)
                <div class="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden transition-all duration-300"
                    ...
                    <div class="flex items-center justify-between p-8 cursor-pointer hover:bg-slate-50 transition-colors"
                        ...
                        <div class="flex items-center gap-8">
                            ...
                            <div>
                                <div class="flex items-center gap-3 flex-wrap">
                                    ...
                                    @if ($period->is_active)
                                        <span
                                            class="inline-flex px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 text-[11px] font-black uppercase tracking-widest border border-emerald-200">
                                            ACTIVE
                                        </span>
                                    @endif
                                    @if ($period->is_sold_out)
                                        <span
                                            class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-100 text-rose-700 text-[11px] font-black uppercase tracking-widest border border-rose-200">
                                ...
                                <p class="text-[12px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                        ...
                        <div class="flex items-center gap-3">
                            {{-- Sold Out Toggle --}}
                            @if ($period->name != 'Invitation & Sponsorship')
                                <form action="{{ route('periods.toggle-sold-out', $period) }}" method="POST"
                                    @click.stop=""
                                ...
                                @if (!$period->is_active)
                                    <form action="{{ route('periods.toggle', $period) }}" method="POST"
                                        ...
                                        @csrf
                                        <button type="submit"
                                            class="bg-[#003366] text-white rounded-md px-6 h-12 text-[11px] font-black uppercase tracking-widest hover:bg-[#002244] transition-all">
                                            Aktifkan Periode
                                        </button>
                                    </form>
                                @endif
                    ...
                    <div x-show="activeAccordion === '{{ $period->id }}'" x-collapse style="display: none;">
                        <div class="p-8 bg-slate-50/50 border-t border-slate-50">
                            <div class="bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm">
                                <table class="w-full text-left">
                                    <thead>
                                        <tr
                                            class="bg-slate-50 uppercase tracking-widest text-[11px] font-black text-slate-400 border-b border-slate-100">
                                            <th class="px-8 py-5">Tipe & Nama</th>
                                            <th class="px-8 py-5 text-center">Harga</th>
                                            <th class="px-8 py-5 text-center">Diskon</th>
                                    ...
                                    <tbody class="divide-y divide-slate-50">
                                        ...
                                        @forelse ($periodTickets as $ticket)
                                            <tr class="hover:bg-slate-50/30 transition-colors group"
                                                ...
                                                <td class="px-8 py-5">
                                                    <div class="flex flex-col gap-2 w-full max-w-[250px]">
                                                        <div class="flex items-center gap-3">
                                                            ...
                                                            <input type="text" x-model="name" @input="isDirty = true"
                                                                @keyup.enter="save()" placeholder="Nama opsional..."
                                                                class="bg-transparent border-none p-0 text-base font-bold text-slate-800 uppercase tracking-tight focus:ring-0 flex-grow">
                                                        </div>
                                                        <div
                                                            class="text-[11px] text-slate-400 font-bold uppercase tracking-[2px] opacity-60 italic pl-1">
                                                            Kategori: {{ $ticket->category->name ?? '-' }}</div>
                                                    </div>
                                                </td>
                                                <td class="px-8 py-5 text-center">
                                                    <div
                                                        class="inline-flex items-center gap-3 px-4 py-2 bg-slate-50 rounded-lg border border-slate-100 focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50 transition-all">
                                                        <span class="text-[11px] font-black text-slate-300">Rp</span>
                                                        <input type="number" x-model="price" @input="isDirty = true"
                                                            @keyup.enter="save()"
                                                ...
                                                <td class="px-8 py-5 text-center">
                                                    <div
                                                        class="inline-flex items-center gap-3 px-4 py-2 bg-rose-50/50 rounded-lg border border-rose-100 focus-within:border-rose-300 focus-within:ring-2 focus-within:ring-rose-50 transition-all">
                                                        <span @class(['text-[11px] font-black text-rose-300'])>Rp</span>
                                                        <input type="number" x-model="discount"
                                                            @input="isDirty = true" @keyup.enter="save()"
                                                ...
                                                <td class="px-8 py-5 text-center">
                                                    <div
                                                        class="inline-flex items-center gap-3 px-4 py-2 bg-slate-50 rounded-lg border border-slate-100 focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50 transition-all">
                                                        <input type="number" x-model="qty" @input="isDirty = true"
                                                            @keyup.enter="save()"
                                                ...
                                                <td class="px-8 py-5 text-center">
                                                    <span @class([
                                                        'inline-flex px-4 py-1.5 rounded-full text-[12px] font-black uppercase tracking-wider',
                                                ...
                                                <td class="px-8 py-5 text-right flex items-center justify-end gap-4">
                                                    <button @click="save()" x-show="isDirty" x-cloak
                                                        ...
                                                        <span x-show="isSaving" class="flex"><svg
                                                    ...
                                                    <div x-show="!isDirty"
                                                        class="text-[11px] font-bold text-slate-300 uppercase italic">
                                                        Terupdate </div>
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="6"
                                                    class="px-6 py-10 text-center text-slate-400 italic text-[10px] font-bold uppercase tracking-widest">
                                                    Tidak ada tiket di periode ini.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            ...
                            <div class="mt-6 p-6 bg-white rounded-xl border border-slate-100 shadow-sm">
                                <form action="{{ route('tickets.store') }}" method="POST"
                                    class="flex items-end gap-4 flex-wrap lg:flex-nowrap">
                                    @csrf
                                    <input type="hidden" name="period_id" value="{{ $period->id }}">
                                    <div class="w-40">
                                        <label
                                            class="text-[11px] font-black text-slate-400 uppercase tracking-widest block mb-2">Tipe</label>
                                    ...
                                    <div class="flex-grow min-w-[200px]">
                                        <label
                                            class="text-[11px] font-black text-slate-400 uppercase tracking-widest block mb-1">Nama
                                    ...
                                    <div class="w-48">
                                        <label
                                            class="text-[11px] font-black text-slate-400 uppercase tracking-widest block mb-1">Kategori</label>
                                    ...
                                    <div class="w-40">
                                        <label
                                            class="text-[11px] font-black text-slate-400 uppercase tracking-widest block mb-1">Harga
                                            (Rp)</label>
                                    ...
                                    <div class="w-32">
                                        <label
                                            class="text-[11px] font-black text-slate-400 uppercase tracking-widest block mb-1">Stok</label>
                                    ...
                                    <button type="submit"
                                        class="px-8 h-14 bg-[#003366] text-white rounded-lg text-[12px] font-black uppercase tracking-widest hover:bg-[#002244] transition-all shadow-sm shadow-blue-100 whitespace-nowrap">
                                        Simpan Tiket
                                    </button>
                                </form>
                            </div>
        ...
        @if (count($periods) === 0)
            <div class="bg-white p-20 rounded-[40px] border border-dashed border-slate-200 text-center">
                ...
                <p class="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">Silakan inisialisasi data
                    periode terlebih dahulu.</p>
            </div>
        @endif
    </div>
</x-layouts.admin>
