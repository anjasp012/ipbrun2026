    ...
    <div x-data="{ sidebarOpen: true }" class="flex min-h-screen">
        ...
        <div :class="sidebarOpen ? 'w-80' : 'w-0 overflow-hidden'"
            ...
            <nav class="flex-1 p-6 space-y-3 mt-6 min-w-[320px]">
                ...
                @if($isAdmin)
                    <a href="{{ route('admin.vouchers.index') }}" class="flex items-center gap-4 p-4 rounded-lg sidebar-item {{ request()->is('admin/vouchers*') ? 'active' : '' }}">
                    ...
                    <a href="{{ route('admin.time-result.index') }}" class="flex items-center gap-4 p-4 rounded-lg sidebar-item {{ request()->is('admin/time-result*') ? 'active' : '' }}">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        <span class="font-bold text-base tracking-wide">Time Results</span>
                    </a>
                @endif
